...
bugs<-read.csv("development_data/andy/bugs.ref.csv")
#bugs <- bugs[bugs$SiteSet == "RefCal" & bugs$SelectedSamples == "Selected", ]
...
source("P:/PartTimers/MarkEngeln/r functions/find_replace.r")
...
gis_data <- read.csv('data/full_station_gis.csv')
gis_data$StationCode <- stations$StationCode
predall<- gis_data
predall$Lat <- stations$New_Lat[match(predall$StationCode, stations$StationCode)]
predall$Long <- stations$New_Long[match(predall$StationCode, stations$StationCode)]
...
#candvar<-c(names(predall)[names(predall) %nin% c("StationCode", "AREA", "PERIMETER", "ECOOMRP075", "LEVEL3")])
...
#sapply(candvar,function(nam)try(hist(predall[,nam],main=nam)));
...
bugs.sub <- bugs[bugs$Family_OTU %nin% c("Exclude", "Ambiguous", "NA"), ]
# bugs.sub400 <- rarify(bugs.sub, "SampleID", "BAResult", 400)
# bugall<- acast(bugs.sub400, SampleID ~ Family_OTU, value.var="BAResult", fun.aggregate=sum)
bugall<- acast(bugs.sub, SampleID ~ Family_OTU, value.var="BAResult", fun.aggregate=sum)
samp <- rep(400, times=nrow(bugall))
samp[rowSums(bugall)<400] <- rowSums(bugall)[rowSums(bugall)<400]  
bugall <- rrarefy(bugall, samp)
...
predall <- merge(
  unique(bugs[, c("StationCode", "SampleID")]), predall, all.x=TRUE
  )

row.names(bugall)== as.character(predall$SampleID)
#samples are not aligned. Fix by aligning bugs data to predictor data, since latter is sorted by sample type;
bugall<-bugall[as.character(predall$SampleID),];
...
predcal <- predall[predall$SampleID %in% bugs$SampleID[bugs$SiteSet == "RefCal" & bugs$SelectedSamples == "Selected"],]  #predictor data -calibration sites;
pred.vld <- predall[predall$SampleID %in% bugs$SampleID[bugs$SiteSet == "RefVal" & bugs$SelectedSamples == "Selected"],]  #predictor data - validation sites;


bugcal<-bugall[predall$SampleID %in% bugs$SampleID[bugs$SiteSet == "RefCal" & bugs$SelectedSamples == "Selected"],] #Bug Abundance matrix, calibration sites;
bugcal.pa<-bugall.pa[predall$SampleID %in% bugs$SampleID[bugs$SiteSet == "RefCal" & bugs$SelectedSamples == "Selected"],]; #Bug presence/absence matrix, calibration sites;
bug.vld.pa<-bugall.pa[predall$SampleID %in% bugs$SampleID[bugs$SiteSet == "RefVal" & bugs$SelectedSamples == "Selected"],] #Bug presence/absence matrix, validation sites;
...
totabun<-apply(bugcal.pa.nonrare,1,sum); #vector of total abundance, each site;
rel.abun<-sweep(bugcal.pa.nonrare,1,totabun,FUN="/"); #relative abundance matrix;
...
clus1<-agnes(x=dissim,diss=T,method="flexible", par.method=betaCalc(-0.25),keep.diss=F,keep.data=F);
...
plot(clus1,which.plots=2,labels=row.names(bugcal.pa.nonrare),cex=.4);
...
# predcal$Lat <- stations$New_Lat[match(predcal$StationCode, stations$StationCode)]
# predcal$Long <- stations$New_Long[match(predcal$StationCode, stations$StationCode)]
predcal$GroupID <- as.factor(grp.id)


# predcal.melt <- melt(predcal, id.var=c("StationCode", "SampleID", "GroupID"))
# predcal.melt <- subset(predcal.melt, variable %in% c("ppt", "temp", "elevation", "Lat", "Long"))
#  predcal.melt$value <- as.numeric(predcal.melt$value) 
# 
# 
# ggplot(predcal.melt, aes(x=GroupID, y=value)) + geom_boxplot() + facet_wrap(~variable, scales="free")
# library(ggmap)
# base <- get_map( as.matrix(geocode("Fresno, CA")), zoom = 6)
# ggmap(base) + geom_point(data=predcal, aes(x=Long, y=Lat)) + facet_wrap(~GroupID)
# 
# 
# plot(predcal$Lat[grps.6==1], predcal$Y_coord[grps.6==1],col='black',
...
#candvar <- c("ppt", "temp", "elevation", "LEVEL3_NAM", names(predcal)[grep("Wt", names(predcal))], "Lat", "Long")
candvar <- c("elevation", "Lat", "Long")

rfdat<-data.frame(predcal[,candvar],Groups=grp.id);
rfdat$Groups<-factor(rfdat$Groups);
rfdat$elevation[is.na(rfdat$elevation)] <- 0
...
rf.mod<-randomForest(Groups ~ elevation + Lat + Long, data=rfdat, ntree=2000, importance=TRUE, norm.votes=TRUE, keep.forest=TRUE);
...
#   partialPlot(rf.mod,pred.data=rfdat,x.var="WtAvgN",which.class=grp,main=paste("Group ",grp))});
# #now click on graph window and use PgUp, PgDn to scroll through plots;
...
# rfdat <- data.frame(rfdat, row.names= as.character(predcal$SampleID))
# 
# OE.assess.cal<-model.predict.RanFor.4.2(bugcal.pa,grps.final,preds.final, ranfor.mod=rf.mod,prednew=rfdat,bugnew=bugcal.pa,Pc=0.5,Cal.OOB=TRUE);
...
# #########;
# 
# predcal$PSA10c <- stations$PSA10c[match(predcal$StationCode, stations$StationCode)]
# merged_scores <- cbind(OE.assess.cal$OE.scores, predcal)
# ggplot(merged_scores, aes(x=elevation, y=OoverE)) + geom_point() +geom_smooth() +geom_hline(yintercept=1-2*c(0.13, 0.1790049, 0.2309817))
# summary(aov(OoverE.null ~ PSA10c, data=merged_scores))
# 
# 
