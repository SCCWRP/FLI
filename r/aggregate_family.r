...
aggregate.BMI <- function(x){
  ...
  x <- merge(x, metadata[,c("FinalID", "LifeStageCode", "SAFIT1", "SAFIT2", "TaxonomicLevelCode",
                             "SAFIT1_effortlevel", "SAFIT2_effortlevel", "EffectiveEffortLevel_Family",
                            "EffectiveEffortLevel_Family_effortlevel")],
             by=c("FinalID", "LifeStageCode"), type="left")
  x <- subset(x, SAFIT1 != "Exclude")
  x <- na.omit(x) 


  agglist <- lapply(c("SAFIT1", "EffectiveEffortLevel_Family"), function(effort){
